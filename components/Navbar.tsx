...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── Top navbar ── */}
      <nav className="flex h-16 sm:h-20 fixed w-full items-center justify-between px-4 sm:px-6 z-50 shadow-md"
        style={{ backgroundColor: 'var(--color-secondary)' }}>

        <Link href={session ? "/dashboard" : "/"} className="flex items-center gap-2 no-underline">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm"
            style={{ backgroundColor: 'var(--color-tertiary)', color: 'var(--color-secondary)' }}>SQ</div>
          <span className="text-lg font-black tracking-wide hidden sm:block"
            style={{ color: 'var(--color-tertiary)' }}>Smart Queue</span>
        </Link>
          ...
          {!session ? (
            ...
            <>
              ...
              <div className="hidden sm:flex items-center gap-1">
                    ...
                    {(['Queue', 'Request', 'Track'] as const).map(label => {
                      ...
                      return (
                        <button key={label} onClick={() => router.push(href)}
                          ...
                          style={active(href) ? { backgroundColor: 'rgba(255,255,255,0.22)' } : {}}>
                          {label}
                        </button>
                      )
                    })}
                ...
                {isStaff && (
                  <button onClick={() => router.push('/dashboard')} className="btn btn-ghost text-sm"
                    style={active('/dashboard') ? { backgroundColor: 'rgba(255,255,255,0.22)' } : {}}>
                    Requests
                  </button>
                )}
                {isAdmin && (
        ...
        <nav className="bottom-nav">
          <button onClick={() => router.push('/dashboard')}
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer',
              color: active('/dashboard') ? 'var(--color-secondary)' : '#9CA3AF' }}>
            <span className="text-xl leading-none">{isAdmin ? '📊' : '📋'}</span>
            <span className="text-xs font-semibold">{isAdmin ? 'Overview' : 'Requests'}</span>
            {active('/dashboard') && <div className="w-1 h-1 rounded-full mt-0.5" style={{ backgroundColor: 'var(--color-secondary)' }} />}
          </button>
          <button onClick={() => signOut({ callbackUrl: "/login" })}
            className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2"
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF' }}>
